                  ...
                  <div style={{ position: 'absolute', top: 10, left: 10, display: 'flex', flexDirection: 'column', gap: 4, zIndex: 2 }}>
                    {enr.bundle_title && (
                      <span style={{ background: '#0f172a', color: '#fff', fontSize: 10.5, fontWeight: 700, padding: '3px 8px', borderRadius: 4, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m7.5 4.27 9 5.15"/><path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="m3.3 7 8.7 5 8.7-5"/><path d="M12 22V12"/></svg>
                        {enr.bundle_title}
                      </span>
                    )}
                    {enr.isLockedBatch && (
                    ...
                    {enr.isExpired && (
                      <span style={{ background: '#dc2626', color: '#fff', fontSize: 10.5, fontWeight: 700, padding: '3px 8px', borderRadius: 4, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
...
function EbooksTab({ user }) {
  ...
  const getDownloadUrl = (url, defaultFilename) => {
    if (!url) return '';
    try {
          ...
          <div key={ebook.id} className="ud-course-card" style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div className="ud-course-card-body" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 10, marginTop: 14 }}>
                {ebook.ebook_url ? (
                  <a href={getDownloadUrl(ebook.ebook_url, `${ebook.title.replace(/\s+slug$/i, '')}.pdf`)} download target="_blank" rel="noreferrer" className="ud-card-btn" style={{ textDecoration: 'none', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                ...
                {ebook.bonus_ebook_urls && ebook.bonus_ebook_urls.length > 0 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 6 }}>
                    ...
                    {ebook.bonus_ebook_urls.map((bonus, idx) => (
                      <a 
                        key={idx} 
                        href={getDownloadUrl(bonus.url, `${bonus.name || `Bonus_${idx + 1}`}.pdf`)} 
                        download 
                        target="_blank" 
                        rel="noreferrer" 
                        style={{ 
...
export default function LMSDashboard() {
  ...
  useEffect(() => {
    if (!hasCourses && activeTab === 'learning') {
      setActiveTab('ebooks')
    }
  }, [hasCourses, activeTab])
          ...
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%', padding: '24px 16px' }}>
            ...
            <nav style={{ display: 'flex', flexDirection: 'column', gap: 6, flex: 1 }}>
              {[
                ...
                if (!hasCourses) {
                  return !['learning', 'certificates', 'affiliate'].includes(item.id)
                }
                return true
              }).map(item => (
                <button
